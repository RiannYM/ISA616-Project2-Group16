...
```{r}
data = data |> 
  dplyr::select(year.x, GPA.Range, major1, survey_plans, survey_internships, survey_offers, survey_salary, Gender, IPEDS.Race.Ethnicity, latin_honors, `Combined Bacc and Masters`, major2)

data$survey_internships = as.factor(data$survey_internships)
data$survey_offers = as.factor(data$survey_offers)
data$year.x = as.factor(data$year.x)
data$GPA.Range = as.factor(data$GPA.Range)
data$survey_plans = as.factor(data$survey_plans)
data$ethnicity = as.factor(data$IPEDS.Race.Ethnicity)
...
```{r}
## what do with all the NAs. We cant really do anything with them if we don't have that data --> salary is actually a problem
...
```{r}
p = ggplot(data = gpa_major_sm, aes(x = major1, y = salary, 
                                fill = GPA.Range)) +
  geom_bar(stat = "identity", position = position_dodge(), 
           alpha = 0.75)  +
  labs(x = "Major", y = "Salary") +
  theme(plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(face="bold", colour="lightblue", 
                                    size = 12),
        axis.title.y = element_text(face="bold", colour="lightblue", 
                                    size = 12),
...
Because salary is relatively normal, we feel it would be appropriate to imputate the mean in this dataset. However, due to the volume of missing rows, we should proceed with caution on any insights we pull from salary.
...
```{r}
data = data |> 
  dummy_cols(select_columns = 'major2')
```

```{r}
data = data |> 
  dplyr::select(year.x, GPA.Range, major1, survey_plans, survey_internships, survey_offers, survey_salary, Gender, IPEDS.Race.Ethnicity, latin_honors, `Combined Bacc and Masters`, major2_Entrepreneurship, major2_none)
```
...
```{r}
# Create a new variable "grouped_internship" based on conditions
data$internship_0 <- ifelse(data$survey_internship == 0, 1, 0)
data$internship_0 <- ifelse(data$survey_internship == 0, 1, 0)
```
